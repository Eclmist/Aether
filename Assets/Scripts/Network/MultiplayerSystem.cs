...
public class MultiplayerSystem : MonoBehaviour
{
	...
	[SerializeField]
	private ushort m_NATServerPort = 15941;
	[SerializeField]
	...
	public void Connect()
	{
		...
		else
		{
			...
			if (m_NATServerHost.Trim().Length == 0)
				...
				((UDPClient)client).Connect(m_IPAddress.text, (ushort)port, m_NATServerHost, m_NATServerPort);
		}
	...
	public void ConnectToMatchmaking()
	{
		...
		if (m_Mgr == null && m_NetworkManager == null)
			throw new System.Exception("A network manager was not provided, this is required for the tons of fancy stuff");

		m_Mgr = Instantiate(m_NetworkManager).GetComponent<NetworkManager>();

		m_Mgr.MatchmakingServersFromMasterServer(m_MasterServerHost, m_MasterServerPort, m_MyElo, (response) =>
		{
			m_Matchmaking = false;
	...
	public void Host()
	{
		...
		else
		{
			...
			if (m_NATServerHost.Trim().Length == 0)
				...
				((UDPServer)m_Server).Connect(port: ushort.Parse(m_PortNumber.text), natHost: m_NATServerHost, natPort: m_NATServerPort);
		}
	...
	public void Connected(NetWorker networker)
	{
		...
		if (m_Mgr == null && m_NetworkManager == null)
		{
			...
			m_Mgr = m_NetworkManager.AddComponent<NetworkManager>();
		}
		else if (m_Mgr == null)
			m_Mgr = Instantiate(m_NetworkManager).GetComponent<NetworkManager>();
		...
		if (!string.IsNullOrEmpty(m_MasterServerHost))
		{
			...
			masterServerData = m_Mgr.MasterServerRegisterData(networker, serverId, serverName, type, mode, comment, m_UseElo, m_EloRequired);
		}

		m_Mgr.Initialize(networker, m_MasterServerHost, m_MasterServerPort, masterServerData);
