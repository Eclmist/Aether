...
public class AnimationCallbacks : MonoBehaviour
{
    ...
    public void DashForward(float distance)
    {
        ...
        NetworkManager.Instance.InstantiateSwordSlash(index: 1, position: transform.position, rotation: Quaternion.LookRotation(m_PlayerStance.GetWeaponTransform().forward, m_PlayerStance.GetWeaponTransform().up));
